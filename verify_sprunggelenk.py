...
def check_report(final, label):
    ...
    header = next((l.strip().rstrip(".") for l in befund.splitlines() if l.strip()), "")
    checks = [
...
val_prompt = m_val.group(1).replace("${rawDictation}", corrected).replace("${generatedReport}", report_b)
